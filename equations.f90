...
!Gauge-dependent perturbation equations
        ...
        !Supported scalar initial condition flags
         ...
         initial_string=6
         integer, parameter :: initial_nummodes =  initial_string !initial_iso_neutrino_vel
...
!precalculated arrays
       ...
       contains
        ...
        subroutine SetupScalarArrayIndices(EV, max_num_eqns)
          ...
          ! AJM - string conservation equation
          EV%s_ix = neq+1
          neq=neq+1 
          maxeq=maxeq+1
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        real(dl) ISW
         ...
         ! AJM
        real(dl) emt00,emt00dot,emtS,emtSdot,emtD,emtP,ktau
        real(dl) evec(2),devec(2)
        ...
        emtD=0.0d0
        emtP=0.0d0
        if (do_strings .and. do_string_source) then
           ktau = k*tau
           if(no_evolve) then
              call get_scalar_evec(1,EV%emode,ktau,evec,devec)
              emt00 = evec(1)/(sqrt(tau)*ktau**weighting)*sqrt(ss_eval(1,EV%emode))
              emtS = evec(2)/(sqrt(tau)*ktau**weighting)*sqrt(ss_eval(1,EV%emode))
              emt00dot = (devec(1)-(weighting+0.5d0)*evec(1))/((tau**1.5d0)*ktau**weighting)*sqrt(ss_eval(1,EV%emode))
              emtSdot = (devec(2)-(weighting+0.5d0)*evec(2))/((tau**1.5d0)*ktau**weighting)*sqrt(ss_eval(1,EV%emode))
           else  
              call get_scalar_evec(EV%q_ix,EV%emode,ktau,evec,devec)
              emt00 = evec(1)/(sqrt(tau)*ktau**weighting)*sqrt(ss_eval(EV%q_ix,EV%emode))
              emtS = evec(2)/(sqrt(tau)*ktau**weighting)*sqrt(ss_eval(EV%q_ix,EV%emode))
              emt00dot = (devec(1)-(weighting+0.5d0)*evec(1))/((tau**1.5d0)*ktau**weighting)*sqrt(ss_eval(EV%q_ix,EV%emode))
              emtSdot = (devec(2)-(weighting+0.5d0)*evec(2))/((tau**1.5d0)*ktau**weighting)*sqrt(ss_eval(EV%q_ix,EV%emode))
           end if
           emtD=y(EV%s_ix)
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        ! Do the pi terms seperately in LOS integral since know pidot
        dgpi = dgpi 
        pidot_sum = pidot_sum 
...
!The rest, note y(9)->octg, yprime(9)->octgprime (octopoles)
    ...
    ! AJM - I'm a little confused about the factor in front of vis - CMBFAST would suggest 2, CAMB 21/10
    ! It doesn't make too much difference
     sources(1) = sources(1) + 8.0*pi*expmmu(j)*(emtSdot - 2.0*adotoa*emtS)/k**2 
     sources(1) = sources(1) + 8.0*pi*vis(j)*21.0/10.0*emtS/k**2
...
!strings
         ...
         ! AJM
         initv(6,i_clxg)= Rc*omtau*(-2._dl/3 + omtau/4)
         initv(6,i_clxr)=initv(2,i_clxg)
         initv(6,i_clxb)=initv(2,i_clxg)*0.75_dl
         initv(6,i_clxc)=initv(2,i_clxb)
         initv(6,i_qg)=-Rc/9*omtau*x
         initv(6,i_qr)=initv(2,i_qg)
         initv(6,i_vb)=0.75_dl*initv(2,i_qg)
         initv(6,i_pir)=-Rc*omtau*x2/3/(2*Rv+15._dl)
         initv(6,i_eta)= Rc*omtau*(1._dl/3 - omtau/8)*EV%Kf(1)
         initv(6,i_aj3r)=0
...
!  Internal variables.
        ...
        !write(*,*) tau,k,emt00,emt00dot,emtS,emtSdot,emtD,emtP,ktau,evec,EV%q_ix
...
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
        ...
        if (CP%Num_Nu_Massive > 0) then
           stop 'Massive neutrinos not supported'
...
!  Evaluate the time derivatives of the vector perturbations, flat case
        ...
        emtV=0.0d0
         if (do_strings .and. do_string_source) then
            ktau = k*tau
            if(no_evolve) then
               call get_vv_evec(1,EV%emode,ktau,evec)
               emtV = (evec*sqrt(vv_eval(1,EV%emode)))/&
                    (sqrt(tau)*ktau**weighting)
            else
               call get_vv_evec(EV%q_ix,EV%emode,ktau,evec)
               emtV = (evec*sqrt(vv_eval(EV%q_ix,EV%emode)))/&
                    (sqrt(tau)*ktau**weighting)
            end if
        end if
...
!  Evaluate the time derivatives of the tensor perturbations.
        ...
        if (do_strings .and. do_string_source) then
            ktau = k*tau
            if(no_evolve) then
               call get_tt_evec(1,EV%emode,ktau,evec)
               emtT = (evec*sqrt(tt_eval(1,EV%emode)))/&
                    (sqrt(tau)*ktau**weighting)
            else
               call get_tt_evec(EV%q_ix,EV%emode,ktau,evec)
               emtT = (evec*sqrt(tt_eval(EV%q_ix,EV%emode)))/&
                    (sqrt(tau)*ktau**weighting)
            end if
        end if
        !write(*,*) tau,emtT
