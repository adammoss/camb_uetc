...
!       P%xi_rad = Ini_Read_Double('xi_rad',0.13d0)
!       P%alpha_rad = Ini_Read_Double('alpha_rad',1.9d0)
!       P%v_rad = Ini_Read_Double('v_rad',0.65d0)
...
!       P%L = Ini_Read_Double('L',0.95d0)
	  ...
	  uetc_file = trim(outroot)//'uetc'
       ...
       ! AJM
       if (do_strings) then
         if (global_error_flag==0) call CAMB_GetResults_string(P,plot_uetc)
	  call output_cl_files_string(ScalarFileName, VectorFilename, TensorFileName, TotalFileName, &
