...
contains
  ...
  subroutine write_string_params(SPR)
    ...
    call get_SPR(0.1d0*ktau_min,SPR)
...
!    no = 350
    ...
    taui = 2.0d-6
    tauf = rombint(dtauda,0.0d0,20.0d0,1.0d-4)
    dtau = log(tauf/taui)/dble(no-1)
    do i = 1,no
        t(i)=taui*exp(dble(i-1)*dtau)
    end do
...
!    close(unit = 55)
  ...
  subroutine get_SPR(tau,SPR)
    ...
    if ((do_VOS3).or.(do_VOS4)) then
             ...
             SPR%mu = 1.0d0
             SPR%xi = 0.13d0 !SPR_rad%xi*ft + SPR_mat%xi*(1.0d0-ft)
             SPR%alpha = 1.9d0 !SPR_rad%alpha
             SPR%v = 0.65d0 !SPR_rad%v
             SPR%L = 0.95d0 !SPR_rad%L
    end if
...
!    end if
  ...
  subroutine get_uetc(tau1,tau2,k,uetc_val)
    ...
    real(DP) :: alpha1,alpha2,mu1,mu2,v1,v2
    ...
    if ((x1.le.xmin) .and. (x2.le.xmin)) then
       ...
       uetc_val(1) = (alpha1*alpha2*mu1*mu2*x1*x2)/(2*k**2*dsqrt(1 - v1**2)*dsqrt(1 - v2**2))
                     !-(alpha1*alpha2*mu1*mu2*(-6.0d0 + rho**2.0d0)*x1*x2)/&
                     !(6.0d0*k**2.0d0*dsqrt((-1.0d0+ v1**2.0d0)*(-1.0d0 +  v2**2.0d0)))

       uetc_val(2) = (mu1*mu2*(rho**2.0d0*(-10.0d0 + (10.0d0 - 11.0d0*alpha2**2.0d0)*v2**2.0d0 + v1**2.0d0*(10.0d0 - &
                     11.0d0*alpha1**2.0d0 + (-10.0d0 + 11.0d0*alpha1**2.0d0 + 11.0d0*(1.0d0 - &
                     2.0d0*alpha1**2.0d0)*alpha2**2.0d0)*v2**2.0d0))+ 42.0d0*(2.0d0 + (-2.0d0 + alpha2**2.0d0)*v2**2.0d0 + &
                     v1**2.0d0*(-2.0d0 +  alpha1**2.0d0 + (2.0d0 - alpha1**2.0d0 + (-1.0d0 + 2.0d0*alpha1**2.0d0)*alpha2**&
                     2.0d0)*v2**2.0d0)))*x1*x2)/(420.0d0*alpha1*alpha2*k**2.0d0*dsqrt((-1.0d0 + v1**2.0d0)*(-1.0d0 +  v2**2.0d0)))

       uetc_val(3) = -(mu1*mu2*Pi*x1*x2* (3*rho**2*(2*(4 + (-4 + 3*alpha2**2)*v2**2)* (-48*(-64 + x2**2) + x1**2*(-48 + x2**2)) + &
                     v1**2*(-2*(4 + (-4 + 3*alpha2**2)*v2**2)* (-48*(-64 + x2**2) + x1**2*(-48 + x2**2)) + alpha1**2* &
                     (6*(-48*(-64 + x2**2) + x1**2*(-48 + x2**2)) +  v2**2*(-6*(-48*(-64 + x2**2) + &
                     x1**2*(-48 + x2**2))+alpha2**2*(33792 - 368*x2**2+x1**2*(-368 +7*x2**2)))))) - &
                     8*(4*(2 + (-2 +alpha2**2)*v2**2)*(-192*(-48+x2**2)+x1**2*(-192+5*x2**2))+v1**2*(-4*(2+(-2+alpha2**2)*v2**2)*(-192*(-48+ &
                     x2**2)+x1**2*(-192+5*x2**2))+alpha1**2*(4*(-192*(-48+x2**2)+x1**2*(-192+5*x2**2))+v2**2*(x1**2*(768-20*x2**2)+ &
                     768*(-48+x2**2)+ alpha2**2*(64512-960*x2**2+x1**2*(-960+23*x2**2))))))))/(1.8874368e7*alpha1*alpha2*k**2*dsqrt(1-v1**2)*dsqrt(1 - v2**2))
                     !(mu1*mu2*(2.0d0 + alpha2**2.0d0*v2**4.0d0 + alpha1**2.0d0*v1**4.0d0*(1.0d0 + 2.0d0*alpha2**2.0d0*v2**4.0d0))&
                     !*x1*x2)/(30.0d0*alpha1*alpha2*k**2.0d0*v1**2.0d0*v2**2.0d0*dsqrt((-1.0d0 + v1**2.0d0)*(-1.0d0 + v2**2.0d0)))
        

       uetc_val(4) = -(mu1*mu2*(-28.0d0 + 6.0d0*rho**2.0d0 + 28.0d0*v1**2.0d0 - 14.0d0*alpha1**2.0d0*v1**2.0d0 - &
                     6.0d0*rho**2.0d0*v1**2.0d0 + alpha1**2.0d0*rho**2.0d0*v1**2.0d0 + 28.0d0*v2**2.0d0 - &
                     14.0d0*alpha2**2.0d0*v2**2.0d0 - 6.0d0*rho**2.0d0*v2**2.0d0 + alpha2**2.0d0*rho**2.0d0*v2**2.0d0 - &
                     28.0d0*v1**2.0d0*v2**2.0d0 + 14.0d0*alpha1**2.0d0*v1**2.0d0*v2**2.0d0 + 14.0d0*alpha2**2.0d0*v1**&
                     2.0d0*v2**2.0d0 - 28.0d0*alpha1**2.0d0*alpha2**2.0d0*v1**2.0d0*v2**2.0d0 + 6.0d0*rho**2.0d0*&
                     v1**2.0d0*v2**2.0d0 - alpha1**2.0d0*rho**2.0d0*v1**2.0d0*v2**2.0d0 - alpha2**2.0d0*rho**2.0d0*&
                     v1**2.0d0*v2**2.0d0 + 2.0d0*alpha1**2.0d0*alpha2**2.0d0*rho**2.0d0*v1**2.0d0*v2**2.0d0)*x1*x2)/&
                     (420.0d0*alpha1*alpha2*k**2.0d0*dsqrt((-1.0d0 + v1**2.0d0)*(-1.0d0 + v2**2.0d0)))

       uetc_val(5) = -(alpha1*mu1*mu2*rho**2.0d0*(1.0d0 + (-1.0d0 + 2.0d0*alpha2**2.0d0)*v2**2.0d0)*x1*x2)/&
                     (30.0d0*alpha2*k**2.0d0*dsqrt((-1.0d0 + v1**2.0d0)*(-1.0d0 + v2**2.0d0)))
       ...
       write(*,*) 'Small Approx Failed'        
       write(*,*) tau1,tau2,xi1,xi2,alpha1,alpha2,v1,v2
       end if
    ...
    !00
    a1 = 2.0d0*alpha1*alpha2
    sum = a1*I1
    uetc_val(1) = sum*mu1*mu2*scaling_factor(tau1,tau2,xi1,xi2,L)/&
                  (k**2*dsqrt((-1.0d0+v1**2)*(-1.0d0+v2**2)))

    !Scalar
    a1 = (-216*alpha1**2*alpha2**2*v1**2*v2**2 + 3*rho**2*(-1 + v2**2 + 3*alpha2**2*v2**2 + &
          v1**2*(1 - (1 + 3*alpha2**2)*v2**2 + alpha1**2*(3 + (-3 + 7*alpha2**2)*v2**2))) + &
          k**2*(tau1*v1 - tau2*v2)**2*(11 + (-11 + 7*alpha2**2)*v2**2 + v1**2*(-11 + (11 - &
          7*alpha2**2)*v2**2 + alpha1**2*(7 + (-7 + 11*alpha2**2)*v2**2))))/(16.*alpha1*alpha2*rho**2)    
    a2 = (-3*(-9*alpha1**2*alpha2**2*v1**2*v2**2 + rho**2*(-1 + v2**2 + &
         v1**2*(1 + (-1 + alpha1**2*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)

    a3 = (-9*(1 + (-1 + alpha1**2)*v1**2)*(1 + (-1 + alpha2**2)*v2**2))/(2.*alpha1*alpha2)

    a4 = (-3*(-(alpha2**2*rho**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(-18*alpha2**2*v2**2 + &
         rho**2*(1 + (-1 + 4*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)

    a5 = (3*(-(alpha2**2*rho**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(-18*alpha2**2*v2**2 + &
         rho**2*(1 + (-1 + 4*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)

    a6 = (9*(-(alpha2**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(1 + (-1 + 2*alpha2**2)*v2**2)))/ &
         (2.*alpha1*alpha2)

!    a1 = (-27*alpha1**2*alpha2**2*v1**2*v2**2 + &
!         rho**2*(1 + (-1 + 2*alpha1**2)*v1**2)*(1 + (-1 + 2*alpha2**2)*v2**2))/(2.*alpha1*alpha2*rho**2)
!    a2 = (-3*(-9*alpha1**2*alpha2**2*v1**2*v2**2 + &
!         rho**2*(-1 + v2**2 + v1**2*(1 + (-1 + alpha1**2*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)
!    a3 = (-9*(1 + (-1 + alpha1**2)*v1**2)*(1 + (-1 + alpha2**2)*v2**2))/(2.*alpha1*alpha2) 
!    a4 = (-3*(-(alpha2**2*rho**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(-18*alpha2**2*v2**2 + &
!         rho**2*(1 + (-1 + 4*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)
!    a5 = (3*(-(alpha2**2*rho**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(-18*alpha2**2*v2**2 + &
!         rho**2*(1 + (-1 + 4*alpha2**2)*v2**2))))/(2.*alpha1*alpha2*rho**2)
!    a6 = (9*(-(alpha2**2*(-1 + v1**2)*v2**2) + alpha1**2*v1**2*(1 + (-1 + &
!         2*alpha2**2)*v2**2)))/(2.*alpha1*alpha2)
    sum = a1*I1 + a2*I2 + a3*I3 + a4*I4 + a5*I5 + a6*I6
    uetc_val(2) = sum*mu1*mu2*scaling_factor(tau1,tau2,xi1,xi2,L)/&
                  (k**2*dsqrt((-1.0d0+v1**2)*(-1.0d0+v2**2)))
    ...
    sum = a1*I1 + a2*I2 + a3*I3 + a4*I4 + a5*I5 + a6*I6
    uetc_val(3) = sum*mu1*mu2*scaling_factor(tau1,tau2,xi1,xi2,L)/&
                  (k**2*dsqrt((-1.0d0+v1**2)*(-1.0d0+v2**2)))
    ...
    a4 = (-(alpha2**2.0d0*rho**2.0d0*(-1.0d0 + v1**2.0d0)*v2**2.0d0) + alpha1**2.0d0*v1**2.0d0*&
         (6.0d0*alpha2**2.0d0*v2**2.0d0 - rho**2.0d0*(-1.0d0 + v2**2.0d0)))/(4.0d0*alpha1*alpha2*rho**2.0d0)
    ...
    sum = a1*I1 + a2*I2 + a3*I3 + a4*I4 + a5*I5 + a6*I6
    uetc_val(4) = sum*mu1*mu2*scaling_factor(tau1,tau2,xi1,xi2,L)/&
                  (k**2*dsqrt((-1.0d0+v1**2)*(-1.0d0+v2**2)))
...
!    a1 = (alpha1*(1 + (-1 + 2*alpha2**2)*v2**2))/alpha2
!    a2 = (-3*alpha1*(1 + (-1 + alpha2**2)*v2**2))/alpha2
!    a3 = 0.0
!    a4 = -3*alpha1*alpha2*v2**2
!    a5 = 3*alpha1*alpha2*v2**2
!    a6 = 0.0
    sum = a1*I1 + a2*I2 + a3*I3 + a4*I4 + a5*I5 + a6*I6
    uetc_val(5) = sum*mu1*mu2*scaling_factor(tau1,tau2,xi1,xi2,L)/&
                  (k**2*dsqrt((-1.0d0+v1**2)*(-1.0d0+v2**2)))

       if ((isNan(uetc_val(1))).or.(isNan(uetc_val(2))).or.(isNan(uetc_val(3))).or.(isNan(uetc_val(4))).or.(isNan(uetc_val(5)))) then
       write(*,*) 'The rest failed'
       write(*,*) tau1,tau2,xi1,xi2,alpha1,alpha2,v1,v2
       end if
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function splintxi(tau)
  implicit none
  real(DP), intent(in) :: tau
  real(DP) :: splintxi
  if ((tau.ge.t(1)).and.(tau.le.t(no))) then
          splintxi=splint_nr(t,xi,xipr,tau)/tau
  else if (tau.lt.t(1)) then
          splintxi=xi(1)/t(1)
  else if (tau.gt.t(no)) then
          splintxi=xi(no)/t(no)
  end if
end function
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function splinta(tau)
  implicit none
  real(DP), intent(in) :: tau
  real(DP) :: splinta
  if ((tau.ge.t(1)).and.(tau.le.t(no))) then
        splinta=splint_nr(t,alphaz,alphapr,tau)
  else if (tau.lt.t(1)) then
        splinta=alphaz(1)
  else if (tau.gt.t(no)) then
        splinta=alphaz(no)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function splintv(tau)
  implicit none
  real(DP), intent(in) :: tau
  real(DP) :: splintv
  if ((tau.ge.t(1)).and.(tau.le.t(no))) then
        splintv=splint_nr(ktau_array,vz,vpr,tau)
  else if (tau.lt.t(1)) then
        splintv=vz(1)
  else if (tau.gt.t(no)) then
        splintv=vz(no)
