...
!Derived parameters, not set initially
         ...
         real(dl) cr,cm,g,fkr,fkm,alpha,mu,L
...
!  adotrad gives the relation a(tau) in the radiation era:
           ...
           if (.not.call_again) then
              ...
              aeq = (grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates)))/(grhoc+grhob)
              CP%tau_eq=TimeOfz(1.0d0/aeq-1.0d0)
              CP%tau_s=TimeOfz(100.0d0)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      !Cl transfer function variables
       !values of q for integration over q to get C_ls
        ...
        ! AJM
        real(dl), dimension (:,:,:,:), allocatable :: Cl_scalar_nmodes, Cl_tensor_nmodes, Cl_vector_nmodes
        real(dl), dimension (:,:,:), allocatable :: Cl_scalar_mean, Cl_tensor_mean, Cl_vector_mean
        ...
        ! AJM
        if (CP%WantScalars) then
         if (.not.allocated(Cl_scalar_nmodes)) then
            !deallocate(Cl_scalar_nmodes)
            allocate(Cl_scalar_nmodes(lmin:CP%Max_l, CP%InitPower%nn, C_Temp:C_last,1000))
            Cl_scalar_nmodes = 0.0d0
         end if
        end if

        if (CP%WantVectors) then
         if (.not.allocated(Cl_vector_nmodes)) then
            !deallocate(Cl_vector_nmodes)
            allocate(Cl_vector_nmodes(lmin:CP%Max_l, CP%InitPower%nn, CT_Temp:CT_Cross,1000))
        ...
        if (CP%WantTensors) then
          if (.not.allocated(Cl_tensor_nmodes)) then
             !deallocate(Cl_tensor_nmodes)
             allocate(Cl_tensor_nmodes(lmin:CP%Max_l_tensor, CP%InitPower%nn, CT_Temp:CT_Cross,1000))
             Cl_tensor_nmodes = 0.0d0
          end if
        end if

        if (CP%WantScalars) then
         if (.not.allocated(Cl_scalar_mean)) then
            !deallocate(Cl_scalar_mean)
            allocate(Cl_scalar_mean(lmin:CP%Max_l, CP%InitPower%nn, C_Temp:C_last))
            Cl_scalar_mean = 0.0d0
         end if
        end if

        if (CP%WantVectors) then
         if (.not.allocated(Cl_vector_mean)) then 
            !deallocate(Cl_vector_mean)
            allocate(Cl_vector_mean(lmin:CP%Max_l, CP%InitPower%nn, CT_Temp:CT_Cross))
            Cl_vector_mean = 0.0d0
         end if
        end if

        if (CP%WantTensors) then
          if (.not.allocated(Cl_tensor_mean)) then 
             !deallocate(Cl_tensor_mean)
             allocate(Cl_tensor_mean(lmin:CP%Max_l_tensor, CP%InitPower%nn, CT_Temp:CT_Cross))
             Cl_tensor_mean = 0.0d0
          end if
        end if
       ...
       if (CP%WantScalars .and. CP%WantTensors .and. CP%DoLensing .and. LensTotFile /= '') then
        ...
        ! AJM
        subroutine output_cl_files_string(ScalFile,VecFile,TensFile, TotFile, LensFile, LensTotFile, factor)
        implicit none
        integer in,il
        character(LEN=*) ScalFile, VecFile,TensFile, TotFile, LensFile, LensTotFile
        real(dl), intent(in), optional :: factor
        real(dl) fact
        integer last_C
        ...
        if (present(factor)) then
         ...
         if (CP%WantScalars .and. ScalFile /= '') then
           ...
           do in=1,CP%InitPower%nn
             do il=lmin,min(10000,CP%Max_l)
               write(fileio_unit,trim(numcat('(1I6,',last_C))//'E15.5)')il ,fact*Cl_scalar_mean(il,in,C_Temp:last_C)
             end do
             do il=10100,CP%Max_l, 100
               write(fileio_unit,trim(numcat('(1E15.5,',last_C))//'E15.5)') real(il),&
                       fact*Cl_scalar_mean(il,in,C_Temp:last_C)
             end do
            end do
            close(fileio_unit)
         end if
         ...
         if (CP%WantVectors .and. VecFile /= '') then
           open(unit=fileio_unit,file=VecFile,form='formatted',status='replace')
            do in=1,CP%InitPower%nn
             do il=lmin,CP%Max_l
               write(fileio_unit,'(1I5,4E15.5)')il, fact*Cl_vector_mean(il, in, CT_Temp:CT_Cross)
             end do
            end do
           close(fileio_unit)
        end if
       ...
       if (CP%WantTensors .and. TensFile /= '') then
           open(unit=fileio_unit,file=TensFile,form='formatted',status='replace')
            do in=1,CP%InitPower%nn
             do il=lmin,CP%Max_l_tensor
               write(fileio_unit,'(1I6,4E15.5)')il, fact*Cl_tensor_mean(il, in, CT_Temp:CT_Cross)
             end do
            end do
           close(fileio_unit)
        end if
  
        if (CP%WantTensors .and. CP%WantScalars .and. CP%WantVectors .and. TotFile /= '') then
           open(unit=fileio_unit,file=TotFile,form='formatted',status='replace')
           do in=1,CP%InitPower%nn
             do il=lmin,CP%Max_l_tensor

                write(fileio_unit,'(1I6,4E15.5)')il, fact*(Cl_scalar_mean(il, in, C_Temp:C_E)+ &
                     Cl_tensor_mean(il,in, C_Temp:C_E)+Cl_vector_mean(il,in, C_Temp:C_E)), &
                     fact*(Cl_tensor_mean(il,in, CT_B)+Cl_vector_mean(il,in, CT_B)),&
                     fact*(Cl_scalar_mean(il, in, C_Cross) + Cl_tensor_mean(il, in, CT_Cross)+Cl_vector_mean(il, in, CT_Cross))
             end do
             do il=CP%Max_l_tensor+1,CP%Max_l
                  write(fileio_unit,'(1I6,4E15.5)')il ,fact*Cl_scalar(il,in,C_Temp:C_E), 0._dl, fact*Cl_scalar(il,in,C_Cross)
             end do
       ...
       if (CP%WantScalars .and. CP%WantTensors .and. CP%DoLensing .and. LensTotFile /= '') then
        ...
        end subroutine output_cl_files_string
...
! Calculating the timesteps during recombination.
        ...
        if (CP%want_zdrag .or. CP%want_zstar) then !JH: calculate exact zstar and/or zdrag
           ...
           if (CP%want_zstar) then
              call find_z(optdepth,z_star)
              !write(*,*) TimeOfz(z_star)
           end if
           if (CP%want_zdrag) then 
              call find_z(dragoptdepth,z_drag)
              !write(*,*) TimeOfz(z_drag)
           end if
