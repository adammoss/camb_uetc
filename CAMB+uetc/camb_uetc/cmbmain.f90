...
!     qmax - CP%Max_eta_k/CP%tau0, qmin = qmin0/CP%tau0 for flat case
 ...
 !     Will only be calculated if k*tau < max_etak_xx
      ...
      public cmbmain, ClTransferToCl, InitVars,SetkValuesForSources  !InitVars for BAO hack

contains  

     
      subroutine cmbmain(mode)
        use uetc
      ...
      !write(*,*) CP%WantCls,CP%WantScalars,CP%WantVectors,CP%WantTensors
...
!     Calculation of the CMB sources.
      ...
      if(do_diagonalize) then
         tau_s = CP%tau_s
         ! Diagonalize the UETC
         call diagonalize_uetc(Evolve_q%npoints,Evolve_q%points)
      end if
      ...
      if (CP%WantCls) then
         ...
         !write(*,*) Evolve_q%npoints

         !$OMP PARAllEl DO DEFAUlT(SHARED),SCHEDUlE(DYNAMIC) &
         !$OMP & PRIVATE(EV, q_ix)
         do q_ix= 1,Evolve_q%npoints
             if (global_error_flag==0) call DoSourcek(EV,q_ix,emode)
         end do
         !$OMP END PARAllEl DO
...
!     integrating the sources over time and over k.
  ...
  !     Calculating Cls for every l.
     ...
     subroutine SourceToTransfers(q_ix)
      ...
      function GetTauStart(q,q_ix)
        use uetc
        real(dl), intent(IN) :: q
        ! AJM
        integer, intent(in), optional :: q_ix
        real(dl) taustart, GetTauStart,tau_i,tau_f

        ! AJM
        if (.not. do_strings) then 
...
!     Start when massive neutrinos are strongly relativistic.
            if (CP%Num_nu_massive>0) then
               ...
               ! Set this arbitary for now
               taustart = taustart_string 
            end if

            GetTauStart=taustart
            !if (present(q_ix)) write(*,*) q_ix,q,taustart,CP%tau0

      end function GetTauStart


      subroutine DoSourcek(EV,q_ix,emode)
        integer q_ix,emode
        real(dl) taustart
        type(EvolutionVars) EV
            ...
            taustart = GetTauStart(EV%q,q_ix)
...
!         if (CP%WantScalars .and. CP%Reion%Reionization .and. CP%AccuratePolarization) then
         if (CP%Reion%Reionization .and. CP%AccuratePolarization) then
         ...
         !if (HighAccuracyDefault) dkn2=dkn2/2.0
         ! AJM
...
!!Example code for plotting out variable evolution
    ...
    !   call CreateTxtFile('evolve.txt',1)
         ...
         ! write (50,'(2E15.5)') tau, y(EV%g_ix), y(EV%r_ix)
         !end do
        do j=1,1000
            ...
            write (50,'(7E15.5)') tau, delta, growth, y(3), y(4), y(EV%g_ix), y(1)
        end do
         close(1)
         stop
      end if
